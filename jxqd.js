      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.retCode ===0){
              if(data.data.signStatus===1){
                console.log(`签到成功，已签到${data.data.signDays}天`)
...
function getTaskList() {
  return new Promise((resolve) => {
    $.get(taskUrl("fanxiantask/signhb/query","type=1"), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
        } else {
          data = data.replace('try{Query(','');
          data = data.replace('\n);}catch(e){}','');
          if (safeGet(data)) {
            data = JSON.parse(data);
            if(data.ret ===0){
              for (task of data.commontask) {
                if(task.status===1 && task.task!==''){
                  console.log(`去做${task.taskname}任务`)
                  await doTask(task.task);
                  await $.wait(1000);
                }
              }
            }else{
...
function doTask(id) {
  return new Promise((resolve) => {
    $.get(taskUrl("fanxiantask/signhb/doTask",`task=${id}`), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
        } else {
          data = data.replace('try{Dotask(','');
          data = data.replace('\n);}catch(e){}','');
          if (safeGet(data)) {
            data = JSON.parse(data);
            if(data.errmsg ===""){
