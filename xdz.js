...
function getAllTask(uId, timeout = 0){
        ...
        try {
          ...
          for (let dd of data.data)
          {
            if (!dd.taskFinished)
            {
              ...
              if (dd.taskType==='FOLLOW_SHOP')
                await doFollow(uId,dd.id);
            }
            else
...
function doScan(uId,id,timeout = 0){
  var now=(new Date()).getTime();
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.success)   
          {      
            for (let task of data.data.taskItemList)
            {
                ...
                setTimeout( ()=>{
                  ...
                  $.post(url, async (err, resp, data) => {
                    try {
                      data = JSON.parse(data);
                      if (data.success) {
                        console.log(`第${data.data.userFinishedTimes}次浏览完成`); 
                        if (data.data.userFinishedTimes === data.data.finishNeed)
                          scan = false;
                      }    
                    } catch (e) {
                      $.logErr(e, resp);
...
function doFollow(uId,id,itemId,timeout = 0) {
  var now=(new Date()).getTime();
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {
        "taskType":"FOLLOW_SHOP",
        "taskId":id,
        "uniqueId":uId,
        "channel":4,
        "linkId":"Y2aqxng42hZ0eGxGtbCMiQ",
            ...
            new Promise((resolve) => {
              setTimeout( ()=>{
                body = {
                  ...
                  "channel":4,
                  "linkId":"Y2aqxng42hZ0eGxGtbCMiQ",
                  "itemId":data.data.taskItemList[0].itemId
                }
                url = {
                  url : `${JD_API_HOST}apDoTask&body=${escape(JSON.stringify(body))}&_t=${now}&appid=activities_platform`,
                  headers : {
